...
@contextmanager
def pinned_baseline_lockfile(config: dict[str, object]):
    relative_path = config.get("baselineLockfile")
    if relative_path is None:
        yield
        return
    if not isinstance(relative_path, str):
        fail("baselineLockfile must be a relative path")
    source = ROOT / relative_path
    if not source.is_file():
        fail(f"baseline lockfile is missing: {relative_path}")
...
def main() -> None:
    ...
    with pinned_baseline_lockfile(config):
        ...
        if args.update:
            ...
            if CONFIG_PATH.read_text(encoding="utf-8") != updated:
                ...
                changed = True
            print(
                "api baseline snapshots updated" if changed else "api baseline unchanged"
            )
        else:
